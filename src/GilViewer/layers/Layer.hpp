...
public:
    ...
    virtual bool HasToBeUpdated() const {
        return m_hasToBeUpdated;
    }
    virtual void HasToBeUpdated(const bool update) {
        m_hasToBeUpdated=update;
    }

    virtual bool IsVisible() const {
        return m_isVisible;
    }
    virtual void IsVisible(const bool visible) {
        m_isVisible=visible;
        notifyLayerControl_();
    }

    virtual bool IsTransformable() const {
        return m_isTransformable;
    }
    virtual void IsTransformable(const bool transf) {
        m_isTransformable=transf;
        notifyLayerControl_();
    }
    ...
    void SetNotifyLayerControl( const boost::function<void()> &notifier ) {
        notifyLayerControl_ = notifier;
    }
    void SetNotifyLayerSettingsControl( const boost::function<void()> &notifier ) {
        notifyLayerSettingsControl_ = notifier;
    }

    virtual void ZoomFactor(const double zoomFactor) {
        m_zoomFactor = zoomFactor;
    }
    virtual inline double ZoomFactor() const {
        return m_zoomFactor;
    }
    virtual void TranslationX(const double dx) {
        m_translationX = dx;
    }
    virtual inline double TranslationX() const {
        return m_translationX;
    }
    virtual void TranslationY(const double dy) {
        m_translationY = dy;
    }
    virtual inline double TranslationY() const {
        return m_translationY;
    }

    virtual void Resolution(const double r) {
        m_resolution = r;
    }
    virtual inline double Resolution() const {
        return m_resolution;
    }

    virtual void HasOri( const bool hasOri ) {
        m_hasOri = hasOri;
    }
    virtual bool HasOri() {
        return m_hasOri;
    ...
    virtual double TransparencyMin() const {
        return 0.;
    }
    virtual void TransparencyMax(const double t) {}
    virtual double TransparencyMax() const {
        return 0.;
    }
    ...
    virtual boost::shared_ptr<ColorLookupTable> GetColorLookupTable() {
        return boost::shared_ptr<ColorLookupTable>();
    }
