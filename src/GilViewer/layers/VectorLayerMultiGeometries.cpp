...
void VectorLayerMultiGeometries::Draw(wxDC &dc, wxCoord x, wxCoord y, bool transparent, double zoomFactor, double translationX, double translationY, double resolution) const
{
    ...
    for (unsigned int i=0;i<m_splines.size();++i)
    {
        ...
        for (int j=0;j<n;++j)
        {
            wxPoint p = FromLocal(
                    zoomFactor,translationX,translationY,delta,
                    spline[j].first,spline[j].second);
            points.push_back(p);
        }
    ...
    for (unsigned int i=0;i<m_polygons.size();++i)
    {
        ...
        for (unsigned int j=0;j<m_polygons[i].size();++j)
            points.push_back(FromLocal(
                    zoomFactor,translationX,translationY,delta,
                    m_polygons[i][j].first,m_polygons[i][j].second ));
...
{
    m_inner_color=brush.GetColour();
    m_brush_style=brush.GetStyle();
}
...
{
    m_inner_color=wxColour(red, green, blue, alpha);
    m_brush_style=style;
}

void VectorLayerMultiGeometries::SetPenVectorLayerMultiGeometries(const wxPen &pen)
{
    m_border_color=pen.GetColour();
    m_pen_style=pen.GetStyle();
...
{
    m_border_color=wxColor(red, green, blue, alpha);
    m_pen_style=style;
    m_width=width;
}
