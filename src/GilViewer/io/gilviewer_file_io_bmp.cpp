...
boost::shared_ptr<layer> gilviewer_file_io_bmp::load(const string &filename, const ptrdiff_t top_left_x, const ptrdiff_t top_left_y, const ptrdiff_t dim_x, const ptrdiff_t dim_y)
{
    return load_gil_image<bmp_tag>(filename, point_t(top_left_x, top_left_y), point_t(dim_x, dim_y));
}

void gilviewer_file_io_bmp::save(boost::shared_ptr<layer> layer, const string &filename)
{
