...
template <>
struct tiff_read_support_private<double,gray_t> {
   BOOST_STATIC_CONSTANT(bool,is_supported=true);
   BOOST_STATIC_CONSTANT(int,bit_depth=64);
   BOOST_STATIC_CONSTANT(int,color_type=PHOTOMETRIC_MINISBLACK);
};

template <>
struct tiff_write_support_private<double,gray_t> {
   BOOST_STATIC_CONSTANT(bool,is_supported=true);
   BOOST_STATIC_CONSTANT(int,bit_depth=64);
   BOOST_STATIC_CONSTANT(int,color_type=PHOTOMETRIC_MINISBLACK);
};
...
#if GIL_SUPPORTS_64BITS_FLOAT_IMAGES
            case 64: {
                std::size_t element_size=sizeof(gray64F_pixel_t);
                std::size_t size_to_allocate = (std::max)((std::size_t)view.width(),
                                                          (std::size_t)(TIFFScanlineSize(_tp)+element_size-1)/element_size);
                std::vector<gray64F_pixel_t> row(size_to_allocate);
                for (int y=0;y<view.height();++y) {
                    io_error_if(TIFFReadScanline(_tp,&row.front(), y)!=1);
                    std::transform(row.begin(),row.begin()+view.width(),view.row_begin(y),
                                   color_convert_deref_fn<gray64F_ref_t,typename View::value_type,CC>(_cc));
                }
                break;
            }
#endif
...
public:
    ...
    void apply(const View& view) {
        ...
        ///Didier
        if (!boost::is_integral<typename channel_type<View>::type>::value)
        	io_error_if(TIFFSetField(_tp, TIFFTAG_SAMPLEFORMAT, SAMPLEFORMAT_IEEEFP) != 1);
